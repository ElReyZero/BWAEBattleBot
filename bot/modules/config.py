...
es = Elasticsearch(["elasticsearch host"]) # Elastic search host ip. Needs to be setup if using our elastic search account configuration
world_ref = {"1": "Connery", "10": "Miller", "13": "Cobalt",
             "17": "Emerald", "19": "Jaeger", "25": "Briggs", "40": "SolTech"}
faction_ref = {"1": "VS", "2": "NC", "3": "TR"}

acct_index = 'accounts'
acct_owner = "bwae-jaeger-bot_804807027196166175"
history_index = 'account_history'
max_checkout = 12
...
GAPI_JSON = "bot\google_api_secret_test.json"
...
def get_config(launch_str: str):
    ...
    GAPI_JSON = f"/home/bwae-services/bot/google_api_secret{launch_str}.json"
    ...
    for key in general:
        try:
            if key == "no_account_allowed_roles":
                general[key] = config['General'][key].split(',')
            elif isinstance(general[key], bool):
                general[key] = bool(config['General'][key])
            elif isinstance(general[key], int):
    ...
    for key in channels:
        try:
            if key == "matches" or key == "matches1v1":
    ...
    print(os.getcwd())
    try:
        with open('/home/bwae-services/CHANGELOG.md', 'r', encoding='utf-8') as txt:
