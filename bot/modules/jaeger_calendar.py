...
_secret_file = ""
_continents = {"Amerish":6, "Esamir":8, "Hossin":4, "Indar":2, "Oshur":344}
                ...
                for booked in booked_bases:
                    if type(booked) != list:
                        ...
                        for element in booked:
                            if element is not None and element not in booked_bases_list:
                                booked_bases_list.append(element)
...
def _identify_base_from_name(name, base_class):
    ...
    if name in _continents.keys():
        continent = _continents.get(name)
        pool = base_class.get_pool()
        for base in pool:
            data = base.get_data()
            if str(data["zone_id"]) == str(continent):
                results.append(base)
