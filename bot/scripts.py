...
def push_accounts_to_users():
    ...
    for acc in accs:
        p = Player(int(acc), f"_BWAE_ACC_{acc}")
        p_list.append(p)
        print(acc)
        char_list = [f"BWAExPractice{acc}VS", f"BWAExPractice{acc}TR", f"BWAExPractice{acc}NC"]
