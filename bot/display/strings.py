...
class AllStrings(Enum):
    ...
    REG_CANNOT_REGISTER_NO_ACC = Message("You cannot register yourself without a Jaeger account. Please contact a Battle Bot admin if a BWAE lead authorized you to use practice accounts.\nERROR: ROLES_NOT_AUTHORIZED")
    ...
    CANNOT_SEND_HELP = Message("The bot can't send you the help messages!\nPlease enable 'Allow direct messages from server members' under the privacy tab of the server or 'Allow direct messages' on your privacy settings")
    AWAY_BACK = Message("{} Welcome back!")
    AWAY_GONE = Message("Roles removed for {}", ping=False)
    TRAINING_CONFIG = Message("Choose the configuration for the next fight club/training match")
    JOIN_CANCELED = Message("Join ongoing match canceled!")
    CONFIG_CANCELED = Message("The fight club/training match config has been canceled!")
    CONFIG_TIMEOUT = Message("The fight club/training match config has been canceled due to inactivity!")
    TRAINING_ALREADY_CONFIGURED = Message("Next fight club/training has already been configured, do you want to reconfigure it? (y/n)")
    TRAINING_CONFIG_SUCCESS = Message("Next training's configuration has been successfully set to {} rounds! Use `=fc` or `=tr` to join the queue!")
    ...
    LB_ADDED_1v1 = Message("You've been added to the queue!", embed=embeds.lobby_list_1v1)
    LB_ADDED_TRAINING = Message("You've been added to the fight club/training queue", embed=embeds.lobby_list_training)
    LB_REMOVED = Message("You've been removed from the queue!", embed=embeds.lobby_list)
    LB_REMOVED_1v1 = Message("You've been removed from the queue!", embed=embeds.lobby_list_1v1)
    LB_REMOVED_TRAINING = Message("You've been removed from the fight club/training queue!", embed=embeds.lobby_list_training)
    LB_NOT_IN = Message("You're not in queue!")
    LB_QUEUE = Message("Current players in queue:", embed=embeds.lobby_list_info)
    ...
    LB_FORCED_MATCH_START = Message("Match has been force started!")
    LB_WARNING = Message("{} you will be timed out of the lobby soon! Use `=reset` to remain in the queue!")
    LB_CANNOT_FORCE_START_MATCH = Message("Cannot force start the match! The lobby needs at least 3 people in queue not less!")
    LB_TOO_LONG = Message("{} was removed from the lobby by timeout!", embed=embeds.lobby_list)
    LB_TOO_LONG_1v1 = Message("{} was removed from the 1v1 queue by timeout!", embed=embeds.lobby_list_1v1)
    LB_CLEARED = Message("Lobby has been cleared!", embed=embeds.lobby_list)
    LB_CLEARED_1v1 = Message("1v1 Lobby has been cleared!", embed=embeds.lobby_list_1v1)
    LB_TRAINING_CLEARED = Message("Training lobby has been cleared", embed=embeds.lobby_list_training)
    ...
    FACTION_SELECTED = Message("{} Faction selected!")
    ...
    PK_OK_FACTION = Message("Teams are ready! {} pick a faction with `=pick` `tr`/`vs`/`nc`!",
                            ping=False)
    PK_FACTION = Message("Pick a faction with `=pick` `tr`/`vs`/`nc`!",
                            ping=False)
    ...
    UNKNOWN_ERROR = Message("Something unexpected happened! Please try again or contact staff if it keeps happening.\n"
                            "Details: *{}*")
    MISSING_ARGUMENTS = Message("This command needs a mention! It can't be empty")
    MISSING_ARGUMENTS_2 = Message("This command needs an argument! It can't be empty")
    ROLE_CHECK_ENABLED = Message("`No Account` role check has been enabled")
    ROLE_CHECK_DISABLED = Message("`No Account` role check has been disabled")
    SCORES_DISABLED = Message("Player scores have been disabled.")
    SCORES_ENABLED = Message("Player scores have been enabled.")
    CANNOT_RETRIEVE_ACCOUNT_MATCH_STATUS = Message("Match needs to be waiting to start or ongoing for the account to be retrieved!")
    ...
    DM_STEPS_MATCH_JOIN = Message("How to use the bot:", embed=embeds.steps_to_join_match)
    DM_HELP_REGISTER = Message("Register commands:", embed=embeds.register_help)
    DM_HELP_LOBBY = Message("Lobby commands:",embed=embeds.lobby_help)
    DM_HELP_MATCHES = Message("Match commands:",embed=embeds.match_help)
    SHOW_CURRENT_VARS = Message("These are the bot's current status for its global variables:", embed=embeds.global_variables_display)
    RESTART_BOT_CONFIRMATION = Message("Are you sure you want to restart the bot? (y/n)")
    RESTART_CANCELLED= Message("The bot restart has been canceled!")
    RESTART_CANCELLED_TIMEOUT = Message("The bot restart has been canceled due to timeout!")
    RESTART_STARTED= Message("The bot is restarting...")
    ...
    TRIGGER_ERROR = Message("Triggering intentional error!")
    STATS_WIPED = Message("Player stats have been wiped from the database!")
    RESTART_DATA = Message("Restart Data has been reset")
    SET_RESTART_DATA = Message("Restart Data has been set to {}")
    LAST_MATCH_ID = Message("Restart data contents: {}")
    WRONG_CHANNEL_2 = Message("The command `={}` can't be used in {}")
    NO_PERMISSION = Message("The command `={}` can only be used by staff members!")
    MISSING_ARGUMENT = Message("The command `={}` needs an argument!")
    CHANNEL_INIT = Message("`Bot init`: Correctly hooked in channel <#{}>")
    INVALID_STR = Message("You entered an invalid argument! `{}`")
    INVALID_VALUE = Message("You entered an invalid value!")
    ...
    GLOBAL_INFO = Message("Here is what's going on at the moment:", embed=embeds.global_info)
    ...
    RESTART_DISCORD = Message("Be sure to update your discord before clicking on the rules button!")
    ...
    DISPLAY_STATS = Message("Here are your Jaeger stats:", embed=embeds.player_stats)
    ...
    WRONG_COMMAND_JOIN_TRAINING = Message("If you're trying to join the fight club/training, use `=joinTraining` or `=letmein` instead!")
    ...
    BOT_DM_RECEIVED = Message("Thanks for your message, it was forwarded to the bot staff!", ping=False)
    ...
    PLAYER_NOT_IN_MATCH = Message("Cannot retrieve account, player is not in a match!")
    ...
    ACC_CLOSED = Message("{}'s DMS are locked, couldn't send them a Jaeger account after 3 retries!\nPlease enable 'Allow direct messages from server members' under the privacy tab of the server or 'Allow direct messages' on your privacy settings\nTrying to send the account to staff instead.")
    ACC_LOG = Message("Player [name:{}], [id:{}] will receive {}")
    ACC_GIVING = Message("Sent a Jaeger account for {}!", ping=False)
    PLAYER_HAS_PERSONAL_ACCOUNT = Message("This player has a personal account. It cannot be retrieved")
    PLAYER_NEEDS_TO_ACCEPT_ACCOUNT = Message("The player needs to accept their account for it to be retrieved")
    NO_DATA = Message("No data for this id!")
    ACCOUNT_USAGE = Message("Here is the practice account usage for this user:", embed=embeds.usage)
    DISPLAY_USAGE = Message("<@{}> played {} match{} in the last {}. \n(since {})", ping=False)
    ...
    CLEARING_MATCH_NO_ACC = Message("{} Please enable your DM's and try queuing up again! Clearing match...")
    ...
    RM_TIMEOUT = Message("{} will be muted from BWAE Internals until {}!", ping=False)
    ...
    MUTE_SHOW = Message("You are muted from BWAE Internals until {}!")
    MUTE_FREED = Message("You are no longer muted from BWAE Internals!")
    ...
    CANNOT_END = Message("You cannot end a match that you're not a part of.")
    LOBBY_SIZE_CHANGED = Message("Lobby size changed to {}")
    TRAINING_SIZE_CHANGED = Message("Fight Club/Training lobby size changed to {}")
    LOBBY_SIZE_ERR_NOT_EMPTY = Message("Can't change the lobby size because there's users queued up.")
    WRONG_TYPE_ARGS = Message("Invalid argument. The lobby size can only be a number greater than 2.")

    ROUND_LENGTH_CHANGED = Message("Round length changed to {} minutes.")
    ROUND_LENGTH_ERROR = Message("Invalid argument. Round length can only be a number greater than 0. (Argument must be in minutes)")

    CANNOT_END_MATCH_NOT_PLAYING = Message("Can't end the match, the teams are not even playing!")
    ONGOING_1V1 = Message("There's a 1v1 already going. If you want to join, leave the queue and type `=j1v1` in #{} .")
    CANNOT_JOIN_NOT_PLAYING = Message("Cannot join the match. Match must be running for you to join.\nWait for both teams to ready up and try the command again.")
    CANNOT_JOIN_FULL_1V1 = Message("Cannot join the 1v1. Match is full. If you want to have more than 4 people in the match, you need to use trainings.")
    ONLY_PLAYER_CAN_REACT_TO_JOIN = Message("Only the player that sent the command can click the button!")
    SEND_LOG_FILE = Message("Here are the latest logs:")
    NO_LOGS = Message("No logs were found!")
    CHOOSE_FACTION = Message("Choose the team you want to join:")
    ...
    async def file_send(self, ctx, file, *args):
        if not isinstance(ctx, ContextWrapper):
            ctx = ContextWrapper.wrap(ctx)
        kwargs = self.value.get_elements(ctx, string_args=args)
        return await ctx.send(**kwargs, file=file)
