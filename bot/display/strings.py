...
class AllStrings(Enum):
    ...
    TRAINING_CONFIG = Message("Choose the configuration for the next training match")
    TRAINING_NOT_UNLOCKED = Message("You can't join the training queue since it hasn't been configured yet!")
    JOIN_CANCELED = Message("Join ongoing match canceled!")
    CONFIG_CANCELED = Message("The training match config has been canceled!")
    CONFIG_TIMEOUT = Message("The training match config has been canceled due to inactivity!")
    TRAINING_ALREADY_CONFIGURED = Message("Next training has already been configured, do you want to reconfigure it? (y/n)")
    TRAINING_CONFIG_SUCCESS = Message("Next training's configuration has been successfully set to {} rounds!, use `=tr` to join the queue!")
    ...
    LB_ADDED_TRAINING = Message("You've been added to the training queue", embed=embeds.lobby_list_training)
    ...
    LB_REMOVED_TRAINING = Message("You've been removed from the training queue!", embed=embeds.lobby_list_training)
    ...
    TRAINING_SIZE_CHANGED = Message("Training lobby size changed to {}")
