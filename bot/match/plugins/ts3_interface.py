...
class AudioBot(Plugin):
    ...
    def on_round_over(self, is1v1=False, isTraining=False, matchEnd=False):
        audio_strings = ["round_over"]
        if (self.match.round_no == 1 and not is1v1 and not matchEnd and not isTraining) or (isTraining and self.match.round_no < cfg.general["training_rounds"]):
...
class _TaskAudio:
    ...
    @loop(count=1)
    async def task_audio(self, strings, lobby=False, wait=-1, matchEnd=False, is1v1=False):
        ...
        # Prev fix to prevent audio spam
        #if "5s" in strings or "select_base" in strings or matchEnd or is1v1:
            #if not is1v1 or "select_base" in strings:
                #await sleep(7)
            #clearing = f'{cfg.ts["url"]}/api/bot/template/{self.bot.num}(/clear)'
           # await _send_url(clearing)
