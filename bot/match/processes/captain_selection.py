...
class CaptainSelection(Process, status=MatchStatus.IS_CAPTAIN):
    ...
    def __init__(self, match, p_list, is1v1=False, isTraining=False):
        self.match = match
        self.p_list = p_list
        self.players = dict()
        self.is1v1 = is1v1
        ...
        if not self.is1v1:
            ...
            pass
            #TODO: Figure out if there needs something to happen here
    ...
    async def init(self):
        ...
        if not self.is1v1:
            ...
            await self.add_captain(0, players[0])
            await self.add_captain(1, players[1])
    @Process.public
    ...
    async def add_captain(self, i, player):
        ...
        if all_captains_selected:
            self.match.ready_next_process(self.p_list, is1v1=self.is1v1, isTraining=self.isTraining)
            if not self.is1v1:
                self.auto_captain.cancel()
                self.volunteer_ih.clean()
        if not self.is1v1:
