    ...
    if len(args) == 2:  # if 2 args, it should be "no account", if not, invalid request.
        if args[0] == "no" and args[1] == "account":
            if cfg.general["no_account_role"]:
                if not check_roles(cfg.general["no_account_allowed_roles"], ctx.message.author.roles):
                    await display.REG_CANNOT_REGISTER_NO_ACC.send(ctx)
                    return
...
def check_roles(allowed_roles:list, user_roles:list):
    for role in user_roles:
        for allowed_role in allowed_roles:
            if allowed_role == str(role.id):
                return True
    return False