...
class PlayerScore:
    def __init__(self, p_id, team):
        ...
        if cfg.general["training_rounds"]>2:
            self.__rounds = [False for _ in range(cfg.general["training_rounds"])]
        else:
    ...
    async def db_update_stats(self):
        try:
            ...
            print("A player joined the match late so they won't have this session's stats in mongodb")
