...
function my_start
{
  ...
  if [ -f "commands/running" ]; then
    ...
    mkdir -p ../BattleBot-Data
    mkdir -p ../BattleBot-Data/logging
    mkdir -p ../BattleBot-Data/matches
    nohup pipenv run python -u commands/pog_launcher.py >> ../BattleBot-Data/logging/launcher.out 2>&1 &
    echo "Bot started..."
  fi
}
...
function my_discord_log
{
  # Show last logs
  cd "$SCRIPT_DIR/.." || my_error
  tail ../BattleBot-Data/logging/bot_log
}
...
function my_ts3_log
{
  # Show last logs
  cd "$SCRIPT_DIR/.." || my_error
  tail ../BattleBot-Data/logging/ts3_bot.out
}
